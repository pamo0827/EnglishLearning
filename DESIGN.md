...
余白は要素を包む行が持つ。ボタン自身に `margin` を置くと、包んだ行の余白と二重にかかる。

**再生ボタンと再生速度は別の行に置く。** どちらも `inline-flex` なので、あいだに何も挟まらない
画面（結果画面）では横に並んでしまう。それぞれをブロック要素で包んで行を分ける。
性質の違う操作（音を出す／出し方を変える）が横並びになると、同じ種類の選択肢に見える。
...
**画面の骨格はディクテーションと同じにする。** 2つのモードは同じアプリの一部であって、
別のアプリではない。一覧は**章ごとに1枚のカード**とし、中身は「太字の章名 → `label` の
問題数 → 主ボタン → 升目」の順にする。解答画面と結果画面の見出しも
`typography.question-number` ＋ `label` の組で揃える。

**一覧にセットの題名や語数を並べない。** 一覧の役割は「どこから入るか」を選ばせることで、
その判断に必要なのは番号と前回の成績だけ。文書の種類や語数は、入ってから見出しで示せば
足りる。一覧に情報を積むと、章が増えたときに縦に伸びて選びにくくなる。

ただし**開始ボタンにアイコンは付けない**。ディクテーションの再生アイコンは「音声の練習に
入る」ことを示す記号であり、音を出さない読解に付けると意味が変わる。形を揃えることと、
意味のない記号を足すことは別。
...
章カードには12問の一覧を升目で置き、どの問題からでも入れるようにする。順番どおりに
しか進めないと、途中で中断したときに最初からやり直すことになる。

升目には問題番号と直近の点数を出し、点数で色分けする（`success` / `warning` / `danger`）。
未解答は地の色のまま置く。**未解答を目立たせない。** 空欄が赤いと、まだ手を付けていない
ことが失点のように見える。

升目に見出しは付けない。番号と点数が升目そのものに書いてあるので、「N / 12 問 解答済み」と
上に添えても同じことを二度言うだけになる。

主ボタンは未解答の最初の問題を指す。すでに何問か解いていれば「第5問から続ける」と、
どこへ入るのかを文字で言う。
...
- **Don't** 未着手の状態に警告色を使わない。まだやっていないことは失敗ではない。
