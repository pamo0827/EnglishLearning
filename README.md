...
| 章 | 問題 | テーマ |
| --- | --- | --- |
| 第1章 | 12問 | 日常の予定と言い訳。約束・遅刻・断り |
| 第2章 | 12問 | 言いよどみと本音。前置き・ぼかし・言い直し |
| 第3章 | 12問 | 数字・否定・紛らわしい音の対 |

第3章は連結とは**別種の**聞き取り失敗を集めてある。`thirteen`/`thirty`（強勢の位置）、
`can`/`can't`（母音の強さ）、`want`/`won't`（二重母音）、`Tuesday`/`Thursday`（頭の子音）、
`a quarter to`/`after`（前置詞ひとつで1時間半ずれる）など。連結を鍛えても取れない箇所なので、
章を分けている。
