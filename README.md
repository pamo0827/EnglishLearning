...
| 章 | セット | 設問 | 構成 |
| --- | --- | --- | --- |
| 第1章 | 6 | 20問 | 単一文書5・二重文書1（メール／告知／広告／チャット／記事／メール＋日程表） |
| 第2章 | 6 | 25問 | 単一文書3・二重文書2・三重文書1（請求書／求人／記事／広告＋メール／メール往復／メール＋日程＋社内連絡） |

第2章は本番構成に寄せて複数文書の割合を増やしてある（Part 7 は単一10・二重2・三重3）。

### セットを追加する

`content/reading.ts` の `READING_SETS` に追記して `npm run content:build`。
`READING_CHAPTERS` に章を足すのも忘れないこと。
...
シャドーイング、発音判定。所要時間やランクの履歴。いずれも現在はスコープ外
（残るのは直近の点数・正答率だけで、履歴は持たない）。
